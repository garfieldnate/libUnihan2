...
const UnihanImportData PSEUDOFIELD_IMPORT_DATA[]={
	...
	"([a-hj-twxz]*)(yu)?(y)?(ü)?(u[^[:alnum:]])?(ie)?(ue)?([a-z]*)([1-5])\(([[:digit:]]*)\)"},
	    ...
	    UNIHAN_INVALID_FIELD},
	{"$1$N2{yv}$3$N4{v}$N5{v}$N6{iE}$N7{vE}$8", "$9", "$10", NULL}
    },
	...
	{UNIHAN_FIELD_PINYIN_BASE, UNIHAN_FIELD_PINYIN_TONE, UNIHAN_FIELD_FREQ_RANK,
	    UNIHAN_INVALID_FIELD},
	{"$L1$N2{yv}$L3$N4{v}$N5{v}$N6{iE}$N7{vE}$L8", "$9","$+0", NULL}
    },
    ...
    {UNIHAN_FIELD_kXHC1983, UNIHAN_TABLE_kXHC1983, 
	"([[:digit:]]{4})\.([[:digit:]]{2})([[:digit:]])(\*)?[^[:space:]]*:([a-hj-twxz]*)(yu)?(y)?(ü)?(u ̈])?(ie)?(ue)?([a-z]*)(( ̄)?( ́)?( ̌)?( ̀)?)?",
	{UNIHAN_FIELD_DICT_PAGE_0_PADDED_4, UNIHAN_FIELD_DICT_POSITION, UNIHAN_FIELD_DICT_VARIANT_SERIAL,
	    UNIHAN_FIELD_DICT_UNENCODED, UNIHAN_FIELD_PINYIN_BASE, UNIHAN_FIELD_PINYIN_TONE, 
	    UNIHAN_INVALID_FIELD},
	{"$1","$2","$3","$N4{1,0}", "$5$N6{yv}$7$N8{v}$N9{v}$N10{iE}$N11{vE}$12", 
